...
void sSetCar(CarType ct) {
  ...
  if (cartype == camera)
    ...
    CarDirection = 0;
  if (cartype == electromagnetic) MiddleLineFlag = 1;
...
double sGetASpeed() { return (sGetASpeedL() + sGetASpeedR()) / 2.0; }
...
double sGetSpeed() { return sGetASpeed() * WheelRadius; }

double sGetSpeedL() { return sGetASpeedL() * WheelRadius; }

double sGetSpeedR() { return sGetASpeedR() * WheelRadius; }
...
sVector sGetMagnetic(sVector pos) {
  ...
  for (int i = 0; i < PathNum - 1; i++) {
    if (Distance(Middle[i], pos) > 3.0 && Distance(Middle[i + 1], pos) > 3.0)
      continue;
...
int sGetReedSwitch() {
  ...
  // in oval
  if (((pos - ELineL).GetLen() + (pos - ELineR).GetLen()) >
      sqrt(TRACK_WIDTH_OUT * TRACK_WIDTH_OUT +
           5.0 * 5.0 * CONTROLTIME * CONTROLTIME))
    return 0;
  return 1;
}
...
void sGetGraph(unsigned char graph[GRAPH_HEIGHT][GRAPH_WIDTH]) {
...
#ifdef WIN32
  glReadPixels(0, 0, GRAPH_WIDTH, GRAPH_HEIGHT, GL_RGBA, GL_UNSIGNED_BYTE,
               image);
#else
  glReadPixels(0, 0, GRAPH_WIDTH, GRAPH_HEIGHT, GL_BGRA, GL_UNSIGNED_BYTE,
               image);
#endif
  ...
  for (int i = 0; i < GRAPH_HEIGHT; i++)
    for (int j = 0; j < GRAPH_WIDTH; j++) {
      graph[i][j] =
          (image[i][j][0] + image[i][j][1] + image[i][j][2]) / 3;  // gray
...
void sGetLine(unsigned char line[GRAPH_WIDTH]) {
  ...
  glutSetWindow(WinCar);
#ifdef WIN32
  glReadPixels(0, GRAPH_WIDTH / 2, GRAPH_WIDTH, 1, GL_RGBA, GL_UNSIGNED_BYTE,
               image);
#else
  glReadPixels(0, GRAPH_WIDTH / 2, GRAPH_WIDTH, 1, GL_BGRA, GL_UNSIGNED_BYTE,
               image);
#endif
...
void sEnableCustomWindow() { sEnableCustomWindow(1); }

void sEnableCustomWindow(int num) {
  CustomWindowFlag = 1;
  CustomWindowNum = num;
  if (CustomWindowNum >= CUSTOMWINDOWNUM) CustomWindowNum = CUSTOMWINDOWNUM - 1;
  printf("INIT: Custom window enabled.\n");
}

void sSetDisplayFunc(void f()) { sSetDisplayFunc(f, 0); }
